...
select
	row_number() over(order by p.user_id asc) as sl_no,
	u.username,
	p.user_id,
	p.post_id,
	p.content,
...
select
    row_number() over(order by c.post_id asc) as sl_no,
    u.username,
    c.user_id,
    c.post_id,
    c.comment_id,
...
select 
    row_number() over(order by count(p.post_id) desc) as sl_no,
    u.username,
    p.user_id,
    count(p.post_id)
from posts p
...
#  1. finding user = 3 friends 
SELECT 
    CASE 
        WHEN f.user_id1 = 3 THEN f.user_id2
        WHEN f.user_id2 = 3 THEN f.user_id1
    END AS friend_id
FROM friendships f
...
FROM friendships f
JOIN users u 
    ON (u.user_id = f.user_id1 AND f.user_id2 = 3)
    OR (u.user_id = f.user_id2 AND f.user_id1 = 3)
JOIN likes l 
...
select 
    u.username,
    count(p.content) as content_count
from posts p
join users u on u.user_id = p.user_id
group by u.username ;

# Task 5: Challenge Query 
# 9. Find the most influential user (the user whose posts have the highest total likes + comments).
select 
    u.username,
    count(like_id),
    count(comment_id),
    (count(like_id) + count(comment_id) ) as total
from comments c
